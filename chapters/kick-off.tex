...
\begin{itemize}
    ...
    \item utilizzo dello strumento di containerizzazione \gls{dockerg} per la gestione 
...
\begin{itemize}
    ...
    \item il prodotto deve essere sviluppato usando il \gls{frameworkg}\glsfirstoccur FastAPI
...
sulla lettura della documentazione relativa ai \textit{framework} 
...
\textit{tool} di \textit{crawling}.
Tale \textit{tool} richiedeva come parametri il nome del sito web da cui 
eseguire il \textit{crawling}, la domanda dell'utente in linguaggio naturale 
e il tipo di formato desiderato per la generazione della risposta.

Il sistema avviava un processo di \textit{crawling} che, ricorsivamente, 
...
\begin{itemize}
    ...
    \item adozione del pattern \gls{mvvmg} per il \textit{frontend} allo scopo di 
...
\begin{enumerate}
    \item \textbf{server \gls{mcpg} service}: rappresenta il vero e proprio servizio \gls{mcpg} che espone il \gls{tool} di \textit{crawl} e per ogni passaggio effettuato,
    invia al \textit{frontend} un \textit{event} tramite \gls{sseg};
    \item \textbf{search service}: rappresenta il servizio dedicato alla gestione dei messaggi dell'utente il linguaggio naturale. Il microservizio, dato il contesto, comprende quale \gls{toolg} è richiesto dall'utente, estrae i parametri necessari e chiama l'\textit{endpoint} corretto del microservizio \textit{server MCP};
...
\begin{itemize}
    ...
    la risposta generata con i vari passaggi del processo di \textit{crawling};
...
A titolo esemplificativo, nella Tabella~\ref{tab:esempio-iterazione} viene riportata la scheda di gestione utilizzata per documentare e tracciare le attività della terza settimana, periodo in cui è stato sviluppato il prototipo del sistema di \textit{crawling}.
\begin{table}[htbp]
    \centering
    \small
...
\begin{risk}{Elevata curva di apprendimento tecnologico}
    \riskdescription{L'adozione di un ampio \textit{stack} tecnologico comprendente \textit{framework} e paradigmi non approfonditi nel percorso accademico (FastAPI, protocollo MCP, architettura esagonale e Docker) rischia di sottrarre tempo prezioso alla fase più dispendiosa, ovvero la codifica.}
    \risksolution{Pianificazione di una fase iniziale di formazione teorico-pratica con sviluppo di progetti pilota guidati (\textit{QuickStart}) e di un \textit{Proof of Concept} per validare le competenze prima dello sviluppo definitivo.}
...
\begin{risk}{Volatilità e instabilità dei requisiti}
    ...
    \risksolution{Adozione di una metodologia iterativo-incrementale basata su \textit{timebox} settimanali e canali di comunicazione costanti (\textit{Slack} e allineamenti), permettendo una tempestiva ripianificazione e rimodulazione degli obiettivi operativi.}
    \label{risk:volatilita-requisiti} 
\end{risk}

\begin{risk}{Latenza e limitazioni nell'integrazione di LLM e crawling}
    \riskdescription{L'interazione con i modelli linguistici e i processi ricorsivi di \textit{web crawling} potrebbero introdurre colli di bottiglia prestazionali, problemi di \textit{rate limiting} delle \textit{API}.}
...
\begin{risk}{Non-determinismo delle risposte del modello linguistico (LLM)}
    ...
    \risksolution{Implementazione di tecniche stringenti di \textit{prompt engineering}, definizione di vincoli sintattici rigorosi sull'output del modello (richiedendo formati strutturati, come JSON) e aggiunta di validazione dei dati per intercettare e gestire tempestivamente eventuali risposte non conformi.}
...
\begin{risk}{Complessità di integrazione dell'architettura a microservizi}
    ...
    \risksolution{Configurazione e orchestrazione dell'ambiente di sviluppo tramite \textit{Docker} fin dalle prime fasi e sviluppo sequenziale e incrementale dei singoli microservizi per isolare e risolvere tempestivamente le dipendenze critiche.}
...
\begin{itemize}
    ...
    \item \textbf{Sviluppo di un'interfaccia utente reattiva}: Realizzazione del \textit{frontend} dell'applicazione mediante il \textit{framework} \textit{React} (adottando il pattern MVVM), che includa una pagina per l'interazione in stile chatbot con il modello e la visualizzazione in tempo reale, tramite \gls{sseg}, dei singoli passaggi logici eseguiti dal \textit{crawler}.
...
\begin{itemize}
    ...
    \item \textbf{Integrazione di sistemi di tracing avanzati}: Introduzione e configurazione della piattaforma \textit{LangSmith} all'interno del servizio \textit{MCP}, allo scopo di monitorare, tracciare ed analizzare nel dettaglio il flusso delle chiamate e le performance dei modelli di intelligenza artificiale impiegati.
